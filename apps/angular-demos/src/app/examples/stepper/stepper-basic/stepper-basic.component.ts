  ...
  template: `
	<fds-horizontal-stepper [linear]="true" #stepper>
		<ng-template let-stepperHeader fdsTemplate="header">
			...
			></fds-progress>
			<div class="text--center mb-sm" role="alert" aria-live="polite" aria-atomic="true">Question {{stepperHeader.selectedIndex + 1}} of {{stepperHeader.stepsCount}}</div>
		</ng-template>
		...
		<fds-step [stepControl]="firstFormGroup">
			<form [formGroup]="firstFormGroup" class="w-25 m-auto">
				...
				<div class="text--center pt-small">
					<button class="btn btn__primary" fdsStepperNext>Next</button>
				</div>
			</form>
		</fds-step>
		...
		<fds-step [stepControl]="secondFormGroup">
			<form [formGroup]="secondFormGroup" class="w-25 m-auto">
				...
				<div class="text--center pt-small">
					<button class="btn btn__secondary mr-sm" fdsStepperPrevious>Back</button>
					<button class="btn btn__primary" fdsStepperNext>Next</button>
				</div>
			</form>
		</fds-step>
		...
		<fds-step>
			<div class="w-50 m-auto">
				<p class="text--center my-small h3">You are now done.</p>
				<div class="text--center pt-small">
					<button class="btn btn__secondary mr-sm" fdsStepperPrevious>Back</button>
					<button class="btn btn__primary" (click)="stepper.reset()">Reset</button>
				</div>
			</div>
		</fds-step>
