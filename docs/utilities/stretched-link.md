...
description: Make any HTML element or UiDSystem component clickable by "stretching" a nested link via CSS.
group: utilities
---
...
Cards have `position: relative` by default in UiDSystem, so in this case you can safely add the `.stretched-link` class to a link in the card without any other HTML changes.
