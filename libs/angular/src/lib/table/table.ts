import { Component, OnInit, ChangeDetectionStrategy, ViewEncapsulation, AfterViewInit, AfterContentInit, OnChanges, Input, Output, EventEmitter, ViewChild, ElementRef, ContentChildren, TemplateRef, SimpleChanges, OnDestroy, ChangeDetectorRef, Directive, HostListener, QueryList, NgZone } from '@angular/core';
...
@Component({
    ...
    template: `
        <div #container [ngStyle]="style" [class]="styleClass"
            [ngClass]="{'fds-datatable fds-component': true,
            ...
            <div class="fds-datatable-loading-overlay fds-component-overlay" *ngIf="loading && showLoader">
            ...
            <div class="fds-datatable-wrapper" *ngIf="!scrollable">
                <table role="grid" #table [ngClass]="tableStyleClass" [ngStyle]="tableStyle">
                    ...
                    <tbody class="fds-datatable-tbody" [fdsTableBody]="columns" [fdsTableBodyTemplate]="bodyTemplate"></tbody>
                </table>
            </div>

            <div class="fds-datatable-scrollable-wrapper" *ngIf="scrollable">
               <div class="fds-datatable-scrollable-view" #scrollableView [fdsScrollableView]="columns" [scrollHeight]="scrollHeight"></div>
            </div>
...
export class Table implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges {
    ...
    findParentScrollableView(column) {
        if (column) {
            ...
            while (parent && !DomHandler.hasClass(parent, 'fds-datatable-scrollable-view')) {
    ...
    saveColumnWidths(state) {
        let widths = [];
        let headers = DomHandler.find(this.containerViewChild.nativeElement, '.fds-datatable-thead > tr:first-child > th');
...
@Component({
    ...
    template: `
        <ng-container>
            <ng-template ngFor let-rowData let-rowIndex="index" [ngForOf]="dt.value" [ngForTrackBy]="dt.rowTrackBy">
                <ng-container *ngTemplateOutlet="template; context: {$implicit: rowData, rowIndex: rowIndex, columns: columns}"></ng-container>
            </ng-template>
        </ng-container>
        <ng-container *ngIf="dt.loading">
            <ng-container *ngTemplateOutlet="dt.loadingBodyTemplate; context: {$implicit: columns}"></ng-container>
        </ng-container>
    ...
    template: `
        <div #scrollHeader class="fds-datatable-scrollable-header">
            <div #scrollHeaderBox class="fds-datatable-scrollable-header-box">
                <table class="fds-datatable-scrollable-header-table" [ngClass]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
                    <thead class="fds-datatable-thead">
                        <ng-container *ngTemplateOutlet="dt.headerTemplate; context {$implicit: columns}"></ng-container>
                    </thead>
                </table>
            </div>
        </div>
        <ng-container>
            <div #scrollBody class="fds-datatable-scrollable-body" [ngStyle]="{'max-height': dt.scrollHeight !== 'flex' ? scrollHeight : undefined, 'overflow-y': dt.scrollHeight ? 'scroll' : undefined}">
                <table #scrollTable [class]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
                    <tbody class="fds-datatable-tbody" [fdsTableBody]="columns" [fdsTableBodyTemplate]="dt.bodyTemplate"></tbody>
                </table>
            </div>
        </ng-container>
    `,
...
@Component({
    ...
    template: `
        <span class="fds-sortable-column-icon" [ngClass]="{'icon-arrowupward': sortOrder === 1, 'icon-arrowdownward': sortOrder === -1, 'icon-unfold-more': sortOrder === 0}"></span>
        <span *ngIf="isMultiSorted()" class="fds-sortable-column-badge">{{getMultiSortMetaIndex() + 1}}</span>
    `,
